...
{
/* There is a risk of water condensed into the O2 sensor,
 * so the proper pre-heating procedure must be maintainted.
 * From what Bosch says, it folllows:
 * - enter the CJ125 calibration mode.
 * - apply 2V to the heater for 4 seconds.
 * - apply 8.5V to the heater, and within each consecutive second increase
 *    by 0.4V up to the battery voltage.
 * - store UR value as a PWM reference point, quit CJ125 calibration.
 * - change over to PWM heater control mode.
 *
 * This is explained in files in Doc directory.
 */

	/* Command to enter calibration mode */
	COM_SPI(INIT_REG1_WR || 0x9D);
	/* UBAT is less than 8.5V, hardware problem, ABORT! */
	if (UBAT < 8.5)
		return -1;
	/* two volts are equal to 136 from ADC but for convenience,
	 * lets stick to the float calculation.
	 */
	...
	while (UHTR < UBAT)
	{
		/* o2 sensor preheating sequence, starting from 8.5V
		 * and increasing 0.4V per second.
		 */
		pwm_factor = (UHTR / UBAT) * 255;
		UHTR += 0.4;
		delay(1000);
		analogWrite(this->HTR_PIN, byte(pwm_factor));
	}
	/* end of pre-heating, power off the heater */
	analogWrite(this->HTR_PIN, 0);
	this->Setpoint = analogRead(this->UR_PIN);
	/* quit the calibration mode */
	COM_SPI(INIT_REG1_WR || 0x89);
	return 0;
}
